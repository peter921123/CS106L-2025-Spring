...
public:
  ...
  std::optional<Course> find_course(std::string course_title)
  {
     ...
     * type. */
    auto func = [course_title](const Course& course){
      return course_title == course.title;
    };

    auto iter = std::find_if(courses.begin(), courses.end(), func);
    
    if (iter == courses.end()) return std::nullopt;
  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = course
      .transform([](const Course& course){ return "Found course: " + course.title + "," + course.number_of_units + "," + course.quarter;})
